...
int main(int argc, char** argv)
{
   ...
   // ------------------------------------------------------------------------
   // ilaguna - Load checkpoint
   int loaded = 0;
   if (thereIsACheckpoint()) {
     if(getMyRank() == 0) printf("Loading checkpoint...\n");
     startTimer(chkptLoadTimer);
     loadCheckpoint(sim);
     stopTimer(chkptLoadTimer);
     iStep = sim->iteration;
     loaded = 1;
   }
   // ------------------------------------------------------------------------
   ...
   for (; iStep<nSteps;)
   {
     ...
     if ((iStep % ckptRate)==0 && iStep>0 && !loaded)
     {
       ...
       startTimer(chkptStoreTimer);
       writeCheckpoint(sim);
       stopTimer(chkptStoreTimer);
     }
     loaded = 0;
     // ----------------------------------------------------------------------
     ...
     iStep += printRate;
     sim->iteration = iStep; // ilaguna: save last iteration in Domain struct
   }
